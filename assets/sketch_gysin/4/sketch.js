...
function draw() {
  ...
  // 4. Disegna il percorso ottimo in rosso forte (solo alla fine)
  if (done && path.length > 0) {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (i === 0) {
        // end node (B): punto sul bordo rivolto verso il nodo precedente
        if (path.length > 1) {
          let prevNode = path[1];
          let dx = node.i - prevNode.i;
          let dy = node.j - prevNode.j;
          vertex(cx - dx * w / 2, cy - dy * h / 2);
        }
      } else if (i === path.length - 1) {
        // start node (A): punto sul bordo rivolto verso il nodo successivo
        let nextNode = path[i - 1];
        let dx = nextNode.i - node.i;
        let dy = nextNode.j - node.j;
        vertex(cx + dx * w / 2, cy + dy * h / 2);
      } else {
        vertex(cx, cy);
      }
    ...
    // Disegna la freccia rossa sul bordo di B
    if (path.length > 1) {
      let lastNode = path[1];
      ...
      let bx = end.i * w + w / 2 - dx * w / 2;
      let by = end.j * h + h / 2 - dy * h / 2;
      ...
      fill(239, 68, 68);
      noStroke();
      triangle(-14, -8, 0, 0, -14, 8); // Punta tocca esattamente il bordo (0,0)
      pop();
    }
  }

  // Bordone perimetrale della griglia bianco
  noFill();
  stroke(255);
  strokeWeight(3);
